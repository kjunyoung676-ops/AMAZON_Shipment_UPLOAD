...
<div class="total-block">
  ...
  <span style="margin-left:auto;font-size:11px;opacity:0.6">${new Date().toLocaleDateString('ko-KR')} &nbsp; ${activeSheet||''}</span>
      ...
      {mode==='3'&&(
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>업로드 컨테이너별 분류 · CONTAINER/SEAL 직접 입력 · G.W/CBM(파란 칸)은 PL PDF 업로드 또는 직접 수정 가능 — 지우면 자동계산으로 복귀(해당 시트에만 적용)</span>{Object.keys(gwCbmOverride).length>0&&<button onClick={()=>{if(confirm(`이 시트의 G.W/CBM 보정값 ${Object.keys(gwCbmOverride).length}개를 모두 지우고 자동계산으로 되돌릴까요?`))setGwCbmOverride({})}} style={{fontSize:11,padding:"3px 10px",color:"var(--color-text-danger)",border:"0.5px solid var(--color-border-danger)",borderRadius:"var(--border-radius-md)",background:"transparent",cursor:"pointer"}}>보정값 전체 초기화</button>}<button onClick={expS3} style={{marginLeft:"auto",fontSize:11,padding:"3px 10px"}}>xlsx 저장</button></div>
          <SheetTabs/>

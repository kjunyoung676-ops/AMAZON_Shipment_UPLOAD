...
export default function ShipmentApp() {
  ...
  // G.W/CBM 수동 보정값(PL PDF 업로드 또는 직접 수정) — key: `${container_no}_${sku}`, 해당 시트(BL)에만 적용, master 미변경
  const [gwCbmOverride,setGwCbmOverride]=useState<Record<string,{gw?:number,cbm?:number}>>({})
  ...
  useEffect(()=>{
    ...
    setGwCbmOverride(lsLoad(sheetKey(activeSheet,"gwCbmOverride"),{}))
  },[activeSheet])
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(()=>{if(activeSheet)lsSave(sheetKey(activeSheet,"gwCbmOverride"),gwCbmOverride)},[gwCbmOverride])
  ...
  function rowGW(r:RowData):number{
    const sku=String(r.sku||''); const ov=gwCbmOverride[gwCbmKey(r.container_no as number,sku)]
    if(ov?.gw!==undefined) return ov.gw
    const m=master[sku]||({} as MasterItem)
    return calcGW(r.quantity,parseFloat(String(m.kg))||0)
  }
  function rowCBM(r:RowData):number{
    const sku=String(r.sku||''); const ov=gwCbmOverride[gwCbmKey(r.container_no as number,sku)]
    if(ov?.cbm!==undefined) return ov.cbm
    const m=master[sku]||({} as MasterItem)
    return calcCBM(r.quantity,parseFloat(String(m.bx))||0,parseFloat(String(m.by))||0,parseFloat(String(m.bz))||0)
  }
  function setGwOverride(containerNo:number, sku:string, v:number|undefined){
    const key=gwCbmKey(containerNo,sku)
    setGwCbmOverride(prev=>({...prev,[key]:{...prev[key],gw:v}}))
  }
  function setCbmOverride(containerNo:number, sku:string, v:number|undefined){
    const key=gwCbmKey(containerNo,sku)
    setGwCbmOverride(prev=>({...prev,[key]:{...prev[key],cbm:v}}))
  }
  // PL PDF에서 텍스트 추출 → 약호(SKU) 매칭 라인의 G.W/CBM 숫자를 읽어 해당 컨테이너에만 자동 반영
  async function parsePlPdfAndApply(f:File, containerNo:number, rows:RowData[]){
    ...
    const skusInCtn=[...new Set(rows.map(r=>String(r.sku||'')).filter(Boolean))]
    const lines=text.split('\n')
    let matched=0
    for(const sku of skusInCtn){
      // 실제 상품 SKU(정확 표기)도 함께 탐색 — master[sku].sku 필드
      const realSku=(master[sku]||{} as MasterItem).sku||''
      const lineIdx=lines.findIndex(l=>l.includes(sku)||(realSku&&l.includes(realSku)))
      if(lineIdx===-1)continue
      // 같은 줄 또는 다음 줄까지 포함해 숫자 토큰 수집 (KGS/CBM 표기 무관)
      const scanText=lines[lineIdx]+' '+(lines[lineIdx+1]||'')
      const nums=[...scanText.matchAll(/\d[\d,]*\.?\d*/g)].map(m=>parseFloat(m[0].replace(/,/g,''))).filter(n=>!isNaN(n)&&n>0)
      if(nums.length<2)continue
      // 관례: 뒤에서 두 번째=G.W(KG), 마지막=CBM (PL 표 컬럼 순서상 N.W, G.W, CBM 순으로 끝나는 경우가 많음)
      const cbm=nums[nums.length-1]
      const gw=nums[nums.length-2]
      if(gw>0 && gw<100000) setGwOverride(containerNo,sku,gw)
      if(cbm>0 && cbm<1000) setCbmOverride(containerNo,sku,cbm)
      matched++
    }
    return matched
  }
...
<div class="total-block">
  ...
  <span style="margin-left:auto;font-size:11px;opacity:0.6">${new Date().toLocaleDateString('ko-KR')} &nbsp; ${activeSheet||''}</span>
      ...
      {mode==='3'&&(
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>업로드 컨테이너별 분류 · CONTAINER/SEAL 직접 입력 · G.W/CBM(파란 칸)은 PL PDF 업로드 또는 직접 수정 가능 — 셀 옆 × 로 개별 취소, 아래 버튼으로 전체 취소(해당 시트에만 적용)</span>{Object.keys(gwCbmOverride).length>0&&<button onClick={()=>{if(confirm(`이 시트의 G.W/CBM 보정값 ${Object.keys(gwCbmOverride).length}개를 모두 지우고 자동계산으로 되돌릴까요?`))setGwCbmOverride({})}} style={{fontSize:11,padding:"3px 10px",color:"var(--color-text-danger)",border:"0.5px solid var(--color-border-danger)",borderRadius:"var(--border-radius-md)",background:"transparent",cursor:"pointer"}}>보정값 전체 초기화</button>}<button onClick={expS3} style={{marginLeft:"auto",fontSize:11,padding:"3px 10px"}}>xlsx 저장</button></div>
          <SheetTabs/>
                ...
                <tbody>
                  {(()=>{
                    ...
                    return[...rows.map((r,i)=>{const m=master[String(r.sku)]||({} as MasterItem);const cpp=parseFloat(String(m.cpp))||16;const sk=s2meta[String(r.sku)]||{};const sku=String(r.sku);const ov=gwCbmOverride[gwCbmKey(g.no,sku)];const gw=rowGW(r);const cbm=rowCBM(r);const gwOv=ov?.gw!==undefined,cbmOv=ov?.cbm!==undefined;return(<tr key={g.no+"_"+i} style={{background:i%2===0?"transparent":"var(--color-background-secondary)"}}>
                      ...
                      <td style={{...TD,...(gwOv?IBLU:{}),padding:"2px 6px",minWidth:74,display:"flex",alignItems:"center",gap:2}}><input type="number" step="0.1" value={gw} onChange={e=>{const v=e.target.value;setGwOverride(g.no,sku,v===""?undefined:parseFloat(v))}} onPaste={e=>{const lines=excelPasteLines(e.clipboardData.getData('text'));if(!lines.length)return;e.preventDefault();lines.forEach((val,offset)=>{const tr=flatRows[rowOffset+i+offset];if(!tr)return;const n=val===''?undefined:parseFloat(val.replace(/,/g,''));setGwOverride(tr.container_no as number,String(tr.sku),n===undefined||isNaN(n)?undefined:n)})}} title={gwOv?"PDF/수동 보정값 (지우면 자동계산으로 복귀)":"자동계산값 — 수정하면 이 시트에만 보정 적용"} style={{...INP,textAlign:"right",color:gwOv?"var(--color-text-info)":"var(--color-text-secondary)",fontWeight:gwOv?600:400}}/>{gwOv&&<button onClick={()=>setGwOverride(g.no,sku,undefined)} title="보정값 지우고 자동계산으로 복귀" style={{flex:"none",fontSize:10,lineHeight:1,padding:"1px 4px",border:"none",background:"transparent",color:"var(--color-text-info)",cursor:"pointer"}}>×</button>}</td>
                      <td style={{...TD,...(cbmOv?IBLU:{}),padding:"2px 6px",minWidth:64,display:"flex",alignItems:"center",gap:2}}><input type="number" step="0.01" value={cbm} onChange={e=>{const v=e.target.value;setCbmOverride(g.no,sku,v===""?undefined:parseFloat(v))}} onPaste={e=>{const lines=excelPasteLines(e.clipboardData.getData('text'));if(!lines.length)return;e.preventDefault();lines.forEach((val,offset)=>{const tr=flatRows[rowOffset+i+offset];if(!tr)return;const n=val===''?undefined:parseFloat(val.replace(/,/g,''));setCbmOverride(tr.container_no as number,String(tr.sku),n===undefined||isNaN(n)?undefined:n)})}} title={cbmOv?"PDF/수동 보정값 (지우면 자동계산으로 복귀)":"자동계산값 — 수정하면 이 시트에만 보정 적용"} style={{...INP,textAlign:"right",color:cbmOv?"var(--color-text-info)":"var(--color-text-secondary)",fontWeight:cbmOv?600:400}}/>{cbmOv&&<button onClick={()=>setCbmOverride(g.no,sku,undefined)} title="보정값 지우고 자동계산으로 복귀" style={{flex:"none",fontSize:10,lineHeight:1,padding:"1px 4px",border:"none",background:"transparent",color:"var(--color-text-info)",cursor:"pointer"}}>×</button>}</td>
                    </tr>)}),
